...
python3.8 -m venv pollyenv
source pollyenv/bin/activate
python3.8 -m pip install -U textblob pyssml
python3.8 -m textblob.download_corpora punkt
cd
zip -r9 ~/repo/sam-config/lambda.zip nltk_data/tokenizers
cd  ~/repo/polly_libraries/pollyenv/lib/python3.8/site-packages/
zip -r9 ~/repo/sam-config/lambda.zip *
cd  ~/repo/polly_libraries/pollyenv/lib64/python3.8/site-packages/
zip -r9 ~/repo/sam-config/lambda.zip *
